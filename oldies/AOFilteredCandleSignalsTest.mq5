...
int OnInit()
  {
   ...
   int fh = FileOpen("mt5FileToEdit.csv", FILE_WRITE|FILE_CSV);
   FileClose(fh);
      ...
      if(isHammerUp(open, close, high, low, barRatio) && isLowestLow(1, 2))
        {
         ...
         screenShot("hammerUp");
         int fileHandle = FileOpen("dupa.csv", FILE_WRITE|FILE_CSV);
         if(fileHandle != INVALID_HANDLE)
           {
            FileWrite(fileHandle, DoubleToString(iHigh(Symbol(), PERIOD_CURRENT, 1)));
            FileWrite(fileHandle, DoubleToString(iLow(Symbol(), PERIOD_CURRENT, 1)));
            FileWrite(fileHandle, DoubleToString(iHigh(Symbol(), PERIOD_CURRENT, 1)));
...
void handleOrders()
  {
//---
   int fileHandle = FileOpen("long.csv", FILE_READ|FILE_CSV);
   if(fileHandle != INVALID_HANDLE)
     {
      double price = StringToDouble(FileReadString(fileHandle));
      ...
      printf("%f, %f, %f, %f", price, sl, tp, lot);
      sendLong(price, sl, tp, lot);
      FileClose(fileHandle);
      FileDelete("long.csv");
     }
   fileHandle = FileOpen("short.csv", FILE_READ|FILE_CSV);
   if(fileHandle != INVALID_HANDLE)
     {
      double price = StringToDouble(FileReadString(fileHandle));
      double sl = StringToDouble(FileReadString(fileHandle));
      ...
      double lot = StringToDouble(FileReadString(fileHandle));
      printf("%f, %f, %f, %f", price, sl, tp, lot);
      sendShort(price, sl, tp, lot);
      FileClose(fileHandle);
      FileDelete("short.csv");
     }
  }
...
//+------------------------------------------------------------------+
void sendLong(double price, double sl, double tp, double lot)
  {
...
//+------------------------------------------------------------------+
void sendShort(double price, double sl, double tp, double lot)
  {
//--- declare and initialize the trade request and result of trade request
   MqlTradeRequest request= {};
   MqlTradeResult  result= {};
//--- parameters of request
   request.action   =TRADE_ACTION_DEAL;                     // type of trade operation
   request.symbol   =Symbol();                              // symbol
   request.volume   =lot;                                   // volume of 0.2 lot
